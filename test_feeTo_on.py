...
OTHER_PRIVATE_KEY = bytes.fromhex('ebae884f9cad2c4887f13265e8c10f36713f87a30a3b854f3b2bbf83a00def62')
...
class Wallet:
    ...
    def initFactory(self, contract_address, abi, address):
        contract = self.connection.eth.contract(address=contract_address, abi=abi)
        tx = contract.functions.initUniswapV2Factory(address).buildTransaction({'from': self.address, "nonce": self.nonce, "gasPrice": 1, 'gas': 100000000000})
        signed = web3.eth.Account.signTransaction(tx, self.private_key)
        txid = bytes.hex(self.connection.eth.sendRawTransaction(signed.rawTransaction))
        tx_receipt = self.connection.eth.wait_for_transaction_receipt(txid)
        return tx_receipt

    def setFeeTo(self, contract_address, abi, address):
        contract = self.connection.eth.contract(address=contract_address, abi=abi)
        tx = contract.functions.setFeeTo(address).buildTransaction({'from': self.address, "nonce": self.nonce, "gasPrice": 1, 'gas': 100000000000})
        signed = web3.eth.Account.signTransaction(tx, self.private_key)
        txid = bytes.hex(self.connection.eth.sendRawTransaction(signed.rawTransaction))
        tx_receipt = self.connection.eth.wait_for_transaction_receipt(txid)
        return tx_receipt
...
if __name__ == '__main__':
    ...
    other_wallet = Wallet(OTHER_PRIVATE_KEY, node)
    ...
    resultInitFactory = feed_wallet.initFactory(addressUniswapV2Factory, abiUniswapV2Factory, feed_wallet.address)
    feed_wallet.update_nonce()
    print("InitFactory: " + str(resultInitFactory))

    resultSetFeeTo = feed_wallet.setFeeTo(addressUniswapV2Factory, abiUniswapV2Factory, other_wallet.address)
    feed_wallet.update_nonce()
    print("SetFeeTo: " + str(resultSetFeeTo))
    ...
    # Check
    print(feed_wallet.totalSupply(addressUniswapV2Pair, abiUniswapV2Pair) == MINIMUM_LIQUIDITY + 249750499251388)
    print(feed_wallet.balanceOf(addressUniswapV2Pair, abiUniswapV2Pair, other_wallet.address) == 249750499251388)
    print(feed_wallet.balanceOf(addressLRC20_1, abiLRC20_1, addressUniswapV2Pair) == 1000 + 249501683697445)
    print(feed_wallet.balanceOf(addressLRC20_2, abiLRC20_2, addressUniswapV2Pair) == 1000 + 250000187312969)
